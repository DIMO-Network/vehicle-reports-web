...
export class AppConfigurationComponent extends LitElement {
  ...
  async _handleSubmit(e) {
    ...
    try {
      ...
      localStorage.setItem('dimo_client_id', this.clientId)
      localStorage.setItem('dimo_api_key', this.apiKey)

      // Import DIMO SDK dynamically to avoid issues with SSR
      const { DIMOClient } = await import('@dimo-network/data-sdk')
      
      // Initialize client and get JWT
      const client = new DIMOClient({
        clientId: this.clientId,
        clientSecret: this.apiKey,
      })

      const devJWT = await client.getDeveloperToken()
      
      // Store JWT and timestamp
      const now = Date.now()
      localStorage.setItem('dimo_jwt', devJWT)
      localStorage.setItem('dimo_jwt_timestamp', now.toString())

      // Dispatch configuration success event
      this.dispatchEvent(new CustomEvent('config-success', {
        detail: { jwt: devJWT, clientId: this.clientId },
        bubbles: true
      }))
